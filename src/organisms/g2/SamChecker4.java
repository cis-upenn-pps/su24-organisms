...
public class SamChecker4 implements OrganismsPlayer {
    ...
    private static final int species = 222;
    ...
    public Move move(int foodHere, int energyLeft, boolean foodN, boolean foodE,
        ...
        if (neighborCount == 1) {
            if (neighborN + neighborE + neighborS + neighborW != species - 3){
                return Move.movement(Action.STAY_PUT);
            }
    ...
    private boolean canReproduce(int energyLeft) {
        return energyLeft > game.M() - game.u();
    }
    ...
    private Move movePerpendicularly(int neighborN, int neighborE, int neighborS, int neighborW) {
        ...
        if (neighborE != -1) return Move.movement(Action.WEST);
        if (neighborW != -1) return Move.movement(Action.EAST);

        if (neighborN != -1) return Move.movement(Action.SOUTH);
        if (neighborS != -1) return Move.movement(Action.NORTH);
